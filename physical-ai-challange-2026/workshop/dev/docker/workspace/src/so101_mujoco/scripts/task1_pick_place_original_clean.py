...
def run_task1():
    ...
    current = move_to_pose(sock, current, POSES['pre_grasp'], duration=3.0)
    time.sleep(0.5)
    print("[3/9] GRASP - descending to object...")
    current = move_to_pose(sock, current, POSES['grasp'], duration=2.0)
    time.sleep(0.5)
    print("[4/9] Closing gripper...")
    grasp_closed = list(POSES['grasp'])
    grasp_closed[5] = -0.12453
    current = move_to_pose(sock, current, grasp_closed, duration=1.5)
    time.sleep(2.0)
    print("[5/9] LIFTING object...")
    current = move_to_pose(sock, current, POSES['lift'], duration=2.5)
    time.sleep(0.5)
    print("[6/9] Moving to above target location...")
    current = move_to_pose(sock, current, POSES['pre_place'], duration=3.0)
    time.sleep(0.5)
    print("[7/9] Lowering to place...")
    current = move_to_pose(sock, current, POSES['place'], duration=2.0)
    time.sleep(0.5)
    print("[8/9] Opening gripper - releasing object...")
    place_open = list(POSES['place'])
    place_open[5] = 1.74533
    current = move_to_pose(sock, current, place_open, duration=1.5)
    time.sleep(1.2)
    print("[9/9] Retreating...")
    current = move_to_pose(sock, current, POSES['retreat'], duration=2.5)
    time.sleep(0.5)
